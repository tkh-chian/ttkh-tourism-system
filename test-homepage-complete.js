  ...
  try {
    ...
    if (approved[0].count === 0) {
      ...
      await connection.execute(`
        INSERT INTO products (name, description, base_price, status, merchant_id, category, location, duration, max_participants, images, created_at, updated_at)
        VALUES 
        ('泰国曼谷一日游', '探索泰国首都的魅力，包含大皇宫、玉佛寺等著名景点', 1200, 'approved', 1, '观光旅游', '曼谷', '1天', 20, '[]', NOW(), NOW()),
        ('普吉岛海滩度假', '享受阳光沙滩，体验水上运动的乐趣', 2500, 'approved', 1, '海滩度假', '普吉岛', '3天2夜', 15, '[]', NOW(), NOW()),
        ('清迈文化之旅', '深入了解泰北文化，参观古寺庙和手工艺村', 1800, 'approved', 1, '文化旅游', '清迈', '2天1夜', 12, '[]', NOW(), NOW())
      `);
      console.log('✅ 已创建3个测试产品');
    }
    ...
    try {
      ...
      if (response.data.length > 0) {
        ...
        response.data.forEach((p, i) => {
          console.log(`  ${i+1}. ${p.name} (状态: ${p.status}, 价格: ¥${p.base_price})`);
        });
        console.log('✅ API测试成功');
      } else {

...
async function finalAPITest() {
  ...
  try {
    ...
    console.log('获取商家响应结构:', {
      ...
      isUsersArray: Array.isArray(merchantsResponse.data.data?.users),
      count: merchantsResponse.data.data?.users?.length || 0
    });
    
    if (merchantsResponse.data.success && Array.isArray(merchantsResponse.data.data?.users)) {
      console.log('✅ 获取商家API修复成功');
      const pendingMerchants = merchantsResponse.data.data.users.filter(m => m.status === 'pending');
